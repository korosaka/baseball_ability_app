...
class FielderListFragment : Fragment(), PlayerListAdapterListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        playerList = uDB.getPlayerForList(Constants.TYPE_FIELDER)

        playerListAdapter = PlayerListAdapter(context!!, playerList, this)
        fielder_list_view.adapter = playerListAdapter
    }

    override fun nameClicked(player: PlayerItemData) {
        AnswerQuestionsActivity.playerType = Constants.TYPE_FIELDER
        val uDB = UtilisingDB(context!!, context!!.applicationContext)
        val intent = uDB.getPlayerWithId(player.playerId, Constants.TYPE_FIELDER)
    ...
    override fun buttonClicked(player: PlayerItemData) {
        val uDB = UtilisingDB(context!!, context!!.applicationContext)
        uDB.deletePlayerWithId(player.playerId, Constants.TYPE_FIELDER)
    }
